...
open class DatabaseHelper (context: Context) : ManagedSQLiteOpenHelper(context, "meal.db", null, 1) {

    companion object {
        var instance: DatabaseHelper? = null
        const val TABLE_NAME = "meal_db"
    ...
    fun loadDataOffline() : ArrayList<MealsItemOffline>{
        val item = ArrayList<MealsItemOffline>()
        val db = writableDatabase
        ...
        if (cursor.moveToFirst()){
            for (i in 0 until cursor.count){
                item.add(MealsItemOffline(cursor.getInt(0), cursor.getString(2), cursor.getString(1), getBitmapFromByte(cursor.getBlob(cursor.getColumnIndex("meal_photo")))))
                cursor.moveToNext()
            }
        }
